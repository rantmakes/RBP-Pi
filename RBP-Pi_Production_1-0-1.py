...
class DataLogger:
    def __init__(self):
        ...
        self.csv_writer.writerow(["Timestamp", "Elapsed_Sec", "Bean_Temp", "Exhaust_Temp", "Humidity", "CO2_g_m3"])
...
class ShutdownInterface:
    def __init__(self, shutdown_callback):
        ...
        if not self.pi.connected:
            logging.error("PIGPIO Daemon missing. Button disabled.")
            self.active = False
            return
    ...
    def cleanup(self):
        if self.active:
            self.set_led(False)
            self.cb_sd_push.cancel()
            self.cb_sd_rels.cancel()
            self.pi.stop()
...
class HardwareInterface:
    ...
    def set_led(self, on):
        if self.btn_interface: self.btn_interface.set_led(on)

    def trigger_shutdown(self):
        if self.btn_interface:
            for _ in range(20):
                self.btn_interface.set_led(True); time.sleep(0.05)
                self.btn_interface.set_led(False); time.sleep(0.05)
        self.cleanup()
    ...
    def cleanup(self):
        ...
        if self.btn_interface: self.btn_interface.cleanup()
    ...
    def init_hardware(self):
        # 1. Shutdown Button
        self.btn_interface = ShutdownInterface(self.trigger_shutdown)
...
if __name__ == "__main__":
    ...
    logging.info("Starting RoastProbe v1.0.1 (Sensors Only)...")
    try:
        ble_server.publish() 
