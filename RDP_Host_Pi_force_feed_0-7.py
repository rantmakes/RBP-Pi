...
class MockProbeHost:
    ...
    def send_temps(self):
        ...
        for item in sensor_data:
            # CORRECTED: Ensure this is a Float (JSON Number)
            # Python 'float' serializes to JSON Number (e.g. 200.55)
            # Python 'str' serializes to JSON String (e.g. "200.55")
            val_native = round(float(item["val"]), 2)
        ...
        datagram = {
            ...
            KEY_SERIAL: HOST_SERIAL,
            KEY_EPOCH: current_epoch,
            # Double Encoded Payload container (String)
            # BUT the content inside this string will use Numbers for RPValue
            KEY_PAYLOAD: json.dumps(payload_list)
        }

        # Add Terminator \r\n
        json_str = json.dumps(datagram)
        msg_bytes = json_str.encode('utf-8') + b'\r\n'
        
        # DEBUG: Print the exact payload occasionally to verify format
        if random.random() < 0.1: # Print 10% of packets
             print(f"\nDEBUG OUTGOING: {json_str[:100]}... (check RPValue type)")
    ...
    def run(self):
        print(f"Roastmaster NATIVE FLOAT Host Started.")
        print(f"Serial: {HOST_SERIAL}")
        print(f"Format: JSON Numbers (200.55) + \\r\\n Terminator")
