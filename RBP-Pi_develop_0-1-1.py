...
class HardwareInterface:
    def __init__(self):
        ...
        # We will store the current encoded values here. 
        # The BLE callback will read from this cache.
        self.value_cache = {} 
    ...
    def encode_rbp_value(self, value):
        # [cite_start]RBP Encoding: Float x 10^2, Int32, Little Endian [cite: 127]
        ...
        for probe in self.probes:
            try:
                ...
                if val is not None:
                    ...
                    # Push update to Bluezero
                    # Access the internal service/char wrapper to update value
                    try:
...
if __name__ == "__main__":
    ...
    # 2. Get Bluetooth Adapter (e.g., 'B8:27:EB:...')
    # This finds the first available bluetooth adapter on the Pi
    try:
    ...
    # 3. Create Peripheral
    # This class manages the Application and Advertisement automatically
    ble_server = peripheral.Peripheral(adapter_address, local_name=SERVER_NAME)

    # --- Add RBP Sensing Service ---
    ble_server.add_service(UUID_SERVICE_RBP_SENSING, True)
    ...
    def add_sensor_char(uuid):
        ble_server.add_characteristic(
            ...
            ['read', 'notify'],       # Flags
            ['read'],                 # Permissions
            None,                     # Read Callback (uses internal value)
            None                      # Write Callback
        )
    ...
    ble_server.add_service(UUID_SERVICE_DEVICE_INFO, True)
    ...
    # [cite_start]Serial Number (Required for RBP Uniquing [cite: 202])
    ble_server.add_characteristic(
        UUID_SERVICE_DEVICE_INFO,
    ...
    # We pass the ble_server instance to the update loop so it can push values
    async_tools.add_timer(UPDATE_RATE_MS, lambda: hw.update_sensors(ble_server))

    # 5. Run Server
    logging.info("Starting RoastProbe (Bluezero)...")
    try:
        ble_server.publish() # This starts advertising and the main loop
