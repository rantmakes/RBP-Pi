...
class MockProbeHost:
    ...
    def send_temps(self):
        ...
        # LOGIC CHANGE: Iterate and send individually
        for item in sensor_data:
            
            # Use single-item list
            payload_list = []
            
            # Using Float (Native Number)
            ...
            payload_list.append(event) # List now contains exactly 1 item
            ...
            # Only log the first channel to web to avoid file-lock trashing
            if item["channel"] == 1:
                self.write_web_log(datagram)
            ...
            # Tiny sleep to allow receiver to process individual UDP packets
            time.sleep(0.02) 
            
        print(".", end="", flush=True)

    def run(self):
        print(f"Roastmaster SINGLE-PACKET Host Started.")
        print(f"Serial: {HOST_SERIAL}")
        print(f"Splitting data into {len(self.probes)} individual packets per cycle.")
