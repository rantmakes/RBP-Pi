...
# 2. Secondary Sensor (SCD-41) -> Software I2C (Custom Pins)
...
class ProbeHost:
    def __init__(self):
        ...
        # Each entry acts as a virtual "Probe" mapped to an RDP Channel
        self.probes = []
    ...
    def init_scd41(self):
        try:
            ...
            # Channel 3: Humidity
            # Note: We send this as EVENT_TEMP (Type 3) so Roastmaster graphs it.
            self.probes.append({
                "channel": 3,
                "label": "Humidity",
                "handle": scd,
...
def send_syn(self):
        ...
        # 1. Create the Payload List (Not a string)
        payload_array = [{ KEY_EVENT_TYPE: EVENT_SYN }]
        
        # 2. Build the Datagram Dictionary
        datagram = {
            KEY_VERSION: RDP_VERSION_1_0,
            KEY_SERIAL: HOST_SERIAL,
            KEY_EPOCH: self.send_count,
            # CORRECTED: Assign the list object directly, do not use json.dumps() here
            KEY_PAYLOAD: payload_array 
        }
        
        # 3. Serialize the ENTIRE packet at once
        ...
        for p in self.probes:
            try:
                ...
                if hasattr(p["handle"], "data_ready") and not p["handle"].data_ready:
                     pass # Use previous value if new data isn't ready yet
...
def send_temps(self):
        ...
        for p in self.probes:
            # Logic: If we have a valid float, send it. If not, send None (which becomes JSON null)
            val_to_send = None
            
            if not p["error"] and p["val"] is not None:
                # CORRECTED: Keep as float to ensure JSON output is numeric (e.g. 25.5), not string "25.5"
                # RDP Datasheet requires RPValue to be a Float 
        ...
        datagram = {
            ...
            KEY_SERIAL: HOST_SERIAL,
            KEY_EPOCH: self.send_count,
            # CORRECTED: Assign the list object directly
            KEY_PAYLOAD: payload_list 
        }

        # Serialize the entire object to valid RDP JSON
        msg_bytes = json.dumps(datagram).encode('utf-8')
