...
# --- Timers (in seconds) ---
# Datasheet advises sending data at least once per second 
# We set this to 0.5 (2Hz) to safely meet that requirement.
SYNC_SEND_RATE = 2.0
TEMP_SEND_RATE = 0.5 
...
# 2. Secondary Sensor (SCD-41) -> Software I2C
...
# Meta Types (Temperature) [cite: 381-388]
...
class ProbeHost:
    ...
    def init_scd41(self):
        """Initialize SCD-41 on Software I2C"""
        try:
            ...
            # Channel 2: Exhaust Temp (MetaType 3004)
            self.probes.append({
                "channel": 2,
            ...
            # Channel 3: Humidity -> Mapped to Ambient (MetaType 3005)
            self.probes.append({
                "channel": 3,
            ...
            # Channel 4: CO2 -> Mapped to MET (MetaType 3002)
            self.probes.append({
                "channel": 4,
                "meta_type": META_MET,
                "handle": scd,
    ...
    def write_web_log(self, datagram):
        ...
        # Copy to avoid modifying the UDP packet
        log_data = datagram.copy()
        
        try:
            temp_path = file_path + ".tmp"
            with open(temp_path, 'w') as f:
                # STRICT JSON: separators=(',', ':') removes all whitespace [cite: 478]
    ...
    def send_syn(self):
        ...
        # STRICT JSON: separators=(',', ':') removes all whitespace to match RDP spec [cite: 477-478]
