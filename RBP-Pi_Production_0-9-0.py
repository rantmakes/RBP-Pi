...
# --- PINS ---
PIN_LED_RED  = 19  # Button LED (+)
PIN_SHUTDOWN = 26  # Button Switch (GND)
PIN_ZCD      = 17  
PIN_FAN      = 27  
PIN_HEATER   = 22
PIN_FAN_TACH = 16  # NEW: Fan RPM Input (Via Voltage Divider!)
...
# ==============================================================================
HEATER_CURVE = [(0.8, 9.0), (2.1, 5.0), (2.9, 1.0)]
FAN_CURVE    = [(2.0, 9.0), (3.9, 5.0), (4.7, 1.0)]
...
UUID_CHAR_USER_3     = rbp_uuid("0017") # Fan (Setting)
UUID_CHAR_USER_4     = rbp_uuid("0018") # NEW: Fan RPM
...
class DataLogger:
    def __init__(self):
        ...
        self.csv_writer.writerow(["Timestamp", "Elapsed_Sec", "Bean_Temp", "Exhaust_Temp", "Humidity", "CO2_g_m3", "Heater_Set", "Fan_Speed", "Fan_RPM"])
...
class Tachometer:
    def __init__(self, pi):
        ...
        self.pi.set_mode(PIN_FAN_TACH, pigpio.INPUT)
        self.pi.set_pull_up_down(PIN_FAN_TACH, pigpio.PUD_UP) # Enable Pullup for Open Collector fans
        
        # Watch for pulses
        self.cb_tach = self.pi.callback(PIN_FAN_TACH, pigpio.FALLING_EDGE, self._pulse)

    def _pulse(self, gpio, level, tick):
        self.pulse_count += 1

    def get_rpm(self):
        # Calculate RPM based on pulses since last read
        now = time.time()
        duration = now - self.start_time
        
        if duration >= 1.0: # Update every second
            # Standard PC Fans: 2 Pulses per Revolution
            # RPM = (Pulses / 2) * (60 / Duration)
            self.rpm = int((self.pulse_count / 2) * (60 / duration))
...
class SystemMonitor:
    ...
    def set_led(self, state):
        if self.active: self.pi.write(PIN_LED_RED, 1 if state else 0)
    ...
    # --- Shutdown Logic ---
    def _sd_push(self, gpio, level, tick): self.shutdown_press_start = tick
    ...
    def get_data(self):
        if not self.active: return (0, 0, 0)
        
        # Check Timeout (Auto-Off)
        if pigpio.tickDiff(self.last_zcd_tick, self.pi.get_current_tick()) > 200000:
            return (0, 0, 0)
        ...
        return (h_set, f_set, rpm)

    def _interpolate(self, current_ms, curve):
        if current_ms <= curve[0][0]: return curve[0][1]
...
class HardwareInterface:
    ...
    def set_led(self, on):
        if self.sys_mon: self.sys_mon.set_led(on)
    ...
    def init_sensors(self):
        ...
        self.register_probe("Heater Set", self.sys_mon, lambda pm: pm.get_data()[0], UUID_CHAR_USER_2, "heater")
        self.register_probe("Fan Speed",  self.sys_mon, lambda pm: pm.get_data()[1], UUID_CHAR_USER_3, "fan")
        self.register_probe("Fan RPM",    self.sys_mon, lambda pm: pm.get_data()[2], UUID_CHAR_USER_4, "rpm")
    ...
    def update_sensors(self):
        ...
        if self.logger: self.logger.log_point(log_data)
        self.set_led(False)
        return True 
...
if __name__ == "__main__":
    ...
    # NEW: Fan RPM
    ble_server.add_characteristic(1, 7, UUID_CHAR_USER_4, [0x00]*4, True, ['read', 'notify'], None, None)
    ...
    logging.info("Starting RoastProbe v0.9.0 (with RPM)...")
    try:
        ble_server.publish() 
