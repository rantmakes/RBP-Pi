...
UUID_CHAR_HUMIDITY_1 = rbp_uuid("000b") # Humidity (Requires Descriptor!)
UUID_CHAR_USER_1     = rbp_uuid("0015") # CO2

# Descriptors
# UUID 001c = Humidity Scale Type (Required by RBP Data Sheet Pg 6)
...
class CapturingCharacteristic(OriginalCharacteristic):
    def __init__(self, service_id, char_id, uuid, *args, **kwargs):
        ...
        # The app rejects Humidity sensors if this Descriptor is missing.
        if str(uuid).lower() == UUID_CHAR_HUMIDITY_1.lower():
            logging.info("Detected Humidity Characteristic -> Adding Scale Descriptor...")
            try:
                # Create the Descriptor attached to 'self' (this characteristic)
                # Value [0x01] = Relative Humidity % (RBP Data Sheet Pg 9)
                bluezero.localGATT.Descriptor(
                    self,                       # Parent Characteristic
                    1,                          # Descriptor ID (arbitrary unique)
...
if __name__ == "__main__":
    ...
    # 3. Create Server (Uses patched Characteristic class)
    ...
    # Humidity (This will trigger the auto-descriptor logic)
