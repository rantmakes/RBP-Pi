...
class RoastmasterBLEServer:
    def __init__(self, loop):
        self.loop = loop
        # Initialize Bless Server (Do not pass loop here if using latest bless, but keeping for compatibility)
    ...
    async def setup_gatt(self):
        ...
        await self.server.add_new_characteristic(UUID_SERVICE_DEVICE_INFO, UUID_CHAR_MANUF_NAME, 
                                                 GATTCharacteristicProperties.read, "RBP_Python_Host".encode('utf-8'), GATTAttributePermissions.readable)
    ...
    async def run(self):
        ...
        # FIX: Explicitly define available characteristics for advertising if needed,
        # but standard bless usage is just start_advertising() without the loop.
        logging.info(f"Starting Advertising as '{SERVER_NAME}'...")
        
        try:
            # FIX: Removed self.server.loop argument
            await self.server.start_advertising()
            logging.info("Advertising started successfully.")
        except AttributeError:
            # Fallback for older bless versions or specific backend quirks
            logging.warning("Standard start_advertising failed. Attempting auto-start check.")
            if not await self.server.is_advertising():
                logging.error("Could not start advertising. Check 'bless' version.")
                return
