...
I have completely removed all references to the high-voltage Phase Monitor, optocouplers, and tachometers. It now strictly reflects your "Safe Mode" architecture: external sensors + safe physical controls.
# RoastProbe-Pi (RBP-Pi)
...
This project relies on pigpio for shutdown button timing and blinka for CircuitPython hardware support.
```bash
sudo apt-get update
...
The daemon is required to read the button state accurately in the background.
```bash
...
```bash
cd ~
git clone [https://github.com/YOUR_USERNAME/RBP-Pi.git](https://github.com/YOUR_USERNAME/RBP-Pi.git)
cd RBP-Pi
```
...
ExecStart=/usr/bin/python3 /home/pi/RBP-Pi/RBP-Pi_Production_1-0-1.py
Restart=always
RestartSec=5
 ...
 * The LED will flash rapidly to acknowledge.
 * Release the button.
 * The system will close all log files, stop sensors, and execute sudo shutdown -h now.
 * Once the LED turns off completely, it is safe to remove power.
