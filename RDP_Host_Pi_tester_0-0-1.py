...
# --- Timers (in seconds) ---
SYNC_SEND_RATE = 2.0
TEMP_SEND_RATE = 0.5 # 2Hz Update Rate
...
class MockProbeHost:
    def __init__(self):
        ...
        self.probes = [
            # Channel 1: Simulated Bean Temp (Sine Wave)
            {"channel": 1, "meta_type": META_BT, "base": 200, "amp": 20, "freq": 0.1},
            # Channel 2: Simulated Exhaust Temp (Cosine Wave)
            {"channel": 2, "meta_type": META_EXHAUST, "base": 150, "amp": 15, "freq": 0.1},
            # Channel 3: Simulated Humidity (Random Noise)
            {"channel": 3, "meta_type": META_AMBIENT, "base": 45, "amp": 2, "freq": 1.0},
            # Channel 4: Simulated CO2 (Linear Ramp)
            {"channel": 4, "meta_type": META_MET, "base": 800, "amp": 100, "freq": 0.05}
        ]

    def write_web_log(self, datagram):
        """Writes strictly formatted JSON to file for monitor.html"""
        file_path = "/var/www/html/rdp_packet.json"
        
        # We try to write, but if permission fails (e.g. running on laptop), we skip
        try:
            temp_path = file_path + ".tmp"
            with open(temp_path, 'w') as f:
                # STRICT JSON: No whitespace, single line
                json.dump(datagram, f, separators=(',', ':'))
            os.replace(temp_path, file_path)
        except Exception as e:
            # print(f"Web Log Error: {e}") # Uncomment to debug file permissions
            pass
    ...
    def generate_fake_data(self):
        ...
        for p in self.probes:
            ...
            if p["channel"] == 3: # Random noise for humidity
                ...
                # Ramps from base to base+amp over time
    ...
    def send_temps(self):
        ...
        for item in sensor_data:
            event = {
                ...
                KEY_VALUE: float("{:.2f}".format(item["val"])), # Round to 2 decimals
                KEY_META: item["meta"]
            }
        ...
        # print(f"Sending Mock Temps: {len(payload_list)} items") # Debug
        self.write_web_log(datagram)
    ...
    def run(self):
        ...
        print(f"Waiting for Roastmaster App to acknowledge...")
