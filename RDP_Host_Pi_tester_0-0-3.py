...
# 1. ENTER YOUR IPHONE'S IP ADDRESS HERE (e.g., "192.168.1.50")
TARGET_IP = "192.168.68.62" 
...
class DirectProbeHost:
    def __init__(self):
        ...
        # Setup UDP Socket (Standard, no Multicast options needed for Unicast)
    ...
    def read_incoming(self):
        try:
            ...
            if (packet.get(KEY_VERSION) == RDP_VERSION_1_0 and 
                ...
                print(f"SUCCESS: Handshake ACK Received from {addr[0]}!")
    ...
    def send_syn(self):
        # UNICAST SYN: Send directly to the iPhone IP
        ...
        print(f"Sending DIRECT SYN to {TARGET_IP}...")
        self.write_web_log(datagram)
        
        # Send directly to target
        self.sock.sendto(msg_bytes, (TARGET_IP, SERVER_PORT))
    ...
    def run(self):
        if "XX" in TARGET_IP:
            print("ERROR: You must edit the script and set TARGET_IP to your iPhone's IP address.")
            return

        print(f"Roastmaster DIRECT MOCK Host Started.")
        print(f"Targeting iPhone at: {TARGET_IP}:{SERVER_PORT}")
