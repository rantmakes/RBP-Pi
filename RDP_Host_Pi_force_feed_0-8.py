...
class MockProbeHost:
    def __init__(self):
        ...
        # RESTORED: Counter for Epoch (Matches working 0-0-1 script)
        self.send_count = 0 
    ...
    def send_temps(self):
        ...
        for item in sensor_data:
            # RPValue: Sending as Float (JSON Number)
        ...
        # Debug Output (10% sample)
        if random.random() < 0.1:
            print(f"DEBUG: Epoch {self.send_count} | Payload: {json.dumps(payload_list)[:60]}...")
    ...
    def run(self):
        print(f"Roastmaster GRAPH FIX Host Started.")
        print(f"Serial: {HOST_SERIAL}")
        print(f"Logic: Counter Epoch (0,1,2...) + Float Values (200.55)")
