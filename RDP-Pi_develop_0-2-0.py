...
# --- SENSOR PINS ---
# SCD-41 uses BitBang I2C on these pins
...
UUID_CHAR_USER_1     = rbp_uuid("0015") # CO2 (User Defined 1)
...
# ==============================================================================
# This ensures we can grab the Characteristic objects created by Bluezero
...
class CapturingCharacteristic(OriginalCharacteristic):
    def __init__(self, service_id, char_id, uuid, *args, **kwargs):
        ...
        # Capture the object reference using UUID as the key
...
class HardwareInterface:
    ...
    def register_probe(self, name, handle, read_func, uuid):
        """
        Registers a hardware sensor and maps it to a Bluetooth UUID.
        """
        self.probes.append({
            "name": name,
            "handle": handle,       # The sensor object (e.g. mcp, scd)
            "read_func": read_func, # Function to call to get value
            "uuid": str(uuid).lower()
        })
    ...
    def init_sensors(self):
        ...
        try:
            ...
            logging.info(" - SCD-41 (Exhaust, CO2) OK")
        except Exception as e:
    ...
    def update_sensors(self):
        ...
        for probe in self.probes:
            try:
                # Check data ready if applicable
                ...
                if val is not None:
                    ...
                    if target_uuid in CAPTURED_CHARS:
                        ...
                        # logging.info(f"Updated {probe['name']}: {val}")
...
if __name__ == "__main__":
    ...
    # 3. Create Server
    # The 'CapturingCharacteristic' class will intercept object creation
    ...
    # Add Characteristics (Captured automatically)
    ...
    logging.info("Starting RoastProbe (Hardware Active)...")
    try:
        ble_server.publish() 
